...
def shot_and_download2(dir_name: str):
    """ 撮影とダウンロードを実行する
...
def move_dir(dir_name: str, dst_path: str):
    ...
    cmd = ['mv', dir_name + "/", dst_path + '/']
    try:
        print_timestamp("move: stert")
        res = subprocess.check_call(cmd)
        print_timestamp("move: end")
...
def schedule_run():
    ...
    s.enter(5,  2, shot_and_download, kwargs={'dir_name':dir_name, 'file_name':'pic_01_' + pic_name + '.jpg'})
    s.enter(10,  2, shot_and_download, kwargs={'dir_name':dir_name, 'file_name':'pic_02_' + pic_name + '.jpg'})
    s.enter(15,  2, shot_and_download, kwargs={'dir_name':dir_name, 'file_name':'pic_03_' + pic_name + '.jpg'})
    s.enter(20, 2, move_dir, kwargs={'dir_name': dir_name, 'dst_path': image_save_dir_path})
    s.enter(25, 3, time_adjustment)
    print_timestamp("run!!")
    s.run()
